...
st.title("Dataflow for Use Case")


df_raw = pd.read_excel('2024-01-28_UseCase_maturity_map_for_FSI.xlsx', header=1,
...
sub_unit_choice = st.sidebar.selectbox('Select your Use Case Category:', sub_unit)

use_case = ((df_raw['node_name']).loc[(df_raw['unit'] == unit_choice) &
                                      (df_raw['sub_unit'] == sub_unit_choice)].unique())
...
graph = graphviz.Graph("jupyter_moons", comment="Jupyter moons", engine='dot')
...
for index, row in df_final_enhanced.iterrows():
    graph.node(name=str(row["source"]),
               ...
               shape="box",
               style="filled",
               href="/use_case_details/?uc_id=" + str(row["uc_id_y"]))

    graph.edge(tail_name=str(row["source"]), head_name=str(row["node_name_x"]))
